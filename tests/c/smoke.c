...
static void roll_x(const uint8_t* src, uint8_t* dst, int dx) {
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x < W; ++x) {
            int sx = x - dx;
            if (sx < 0) sx += W;
...
int main(void) {
    ...
    if (n <= 0) {
        fprintf(stderr, "semper_run failed: %d\n", n);
        semper_destroy(eng);
        return 1;
    }
    ...
    if (fabsf(med_u - (float)DX) > 0.5f) {
        fprintf(stderr, "FAIL: median u %.4f not within 0.5 of %d\n", med_u, DX);
        semper_destroy(eng);
        return 1;
    }
